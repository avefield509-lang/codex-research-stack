...
- You may read, call, compare against, or copy content from other local workspaces when the user approves that direction.
- Do not modify any other repository unless the user has explicitly approved that cross-repo change in the current thread.
- Do not add private paths, personal notes, machine-only fixes, credentials, local-only outputs, or development-log details here.
- If adapting content from another repository into this one, keep the result product-facing and sanitized.
